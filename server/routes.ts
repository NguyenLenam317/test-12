...
export async function registerRoutes(app: Express): Promise<Server> {
  // Add middleware to extract deviceId from headers
  app.use(async (req: Request, res: Response, next) => {
    try {
      // Extract device ID from headers or generate a new one
      const deviceId = req.headers['x-device-id'] as string || generateDeviceId();
      
      // If device ID is not in the headers, set it in the response
      if (!req.headers['x-device-id']) {
        res.setHeader('x-device-id', deviceId);
      }
      
      // Store the device ID in the request object for later use
      (req as any).deviceId = deviceId;
      
      // Get the user ID associated with this device, or use the demo user if not found
      let userId = await storage.getUserIdFromDeviceSession(deviceId);
      if (userId === undefined) {
        // Default to demo user if no user is associated with this device
        userId = 0;
        // Register this device with the demo user
  ...
  app.post("/api/chat/message", async (req: Request, res: Response) => {
    try {
      ...
      // Get user ID from the request object (set by middleware)
      const userId = (req as any).userId;
      const deviceId = (req as any).deviceId;
      
      // Log the request for debugging
      console.log(`Chat message from user ${userId} on device ${deviceId}: ${message.substring(0, 50)}...`);
